...
export const submitAttempt = asyncHandler(async(req, res) => {
    ...
    const awarded = await evaluateAndAwardAchievements(req.user.id, {
        quizzesCompleted,
        streakCount,
        hasPerfectScore: percentage === 100,
        groupQuizzesJoined,
    });
...
export const createQuizFromQuestionBank = asyncHandler(async(req, res) => {
    const { chapterId, subjectId, topicId, count = 10, title, difficulty } = req.body;
    ...
    if (bankQuestions.length === 0) {
        throw new ApiError(404, 'No approved questions found for this selection');
    }
    ...
    const quizTitle =
        title ||
        `${firstQ.subject?.bookName || firstQ.subject?.subjectName || 'PTB'} - ${firstQ.chapter?.chapterName || 'Practice'} Quiz`;
        ...
        data: {
            ...
            questions: {
                create: selected.map((q, idx) => ({
                    ...
                    correctAnswer: q[`option${q.correctAnswer}`] || q.correctAnswer,
